...
public class ContratoDAO {
    ...
    private String buildSelect() {
        return "SELECT c.*, p.titulo AS proyecto_titulo, "
                + "uc.nombre_completo AS cliente_nombre, uf.nombre_completo AS freelancer_nombre, "
                + "cal.id AS cal_id, cal.estrellas AS cal_estrellas, cal.comentario AS cal_comentario "
                + "FROM contrato c "
                + "JOIN proyecto p ON c.proyecto_id = p.id "
                + "JOIN cliente cl ON c.cliente_id = cl.id "
                + "JOIN usuario uc ON cl.usuario_id = uc.id "
                + "JOIN freelancer fl ON c.freelancer_id = fl.id "
                + "JOIN usuario uf ON fl.usuario_id = uf.id "
    ...
    private Contrato map(ResultSet rs) throws SQLException {
        ...
        if (fi != null) {
            c.setFechaInicio(fi.toLocalDateTime());
        }
        Timestamp ff = rs.getTimestamp("fecha_fin");
        if (ff != null) {
            c.setFechaFin(ff.toLocalDateTime());
        }
        ...
        if (!rs.wasNull()) {
            c.setCalificacionId(calId);
            c.setCalificacionEstrellas(rs.getInt("cal_estrellas"));
            c.setCalificacionComentario(rs.getString("cal_comentario"));
        }
        return c;
    }
}
