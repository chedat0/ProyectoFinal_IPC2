...
public class SolicitudDAO {
    ...
    public SolicitudHabilidad obtenerHabilidadPorId(int id) throws SQLException {
        String sql = "SELECT * FROM solicitud_habilidad WHERE id = ?";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, id);
    ...
    public SolicitudCategoria obtenerCategoriaPorId(int id) throws SQLException {
        String sql = "SELECT * FROM solicitud_categoria WHERE id = ?";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
